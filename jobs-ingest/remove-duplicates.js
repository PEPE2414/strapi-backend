...
async function getAllJobs() {
  ...
  while (true) {
    try {
      const url = `${STRAPI_URL}/api/jobs?pagination[page]=${page}&pagination[pageSize]=${pageSize}&populate=*`;
...
async function testApiConnection() {
  ...
  try {
    const response = await fetch(`${STRAPI_URL}/api/jobs?pagination[page]=1&pagination[pageSize]=1`, {
      headers: {
        'Authorization': `Bearer ${STRAPI_API_TOKEN}`,
        'Content-Type': 'application/json'
      }

  ...
  try {
    while (page <= maxPages) {
      ...
      for (const url of urls) {
        console.log(`🔄 Scraping Save the Student page ${page}: ${url}...`);
        
        try {
          const { html } = await get(url);
          const $ = cheerio.load(html);
          
          // Find job listings - Save the Student uses various selectors
          const jobElements = $('.job-item, .job-card, .search-result, .job-listing, .graduate-job, .internship-listing');
    ...
    console.log(`📊 Save the Student: Found ${jobs.length} total jobs across ${page - 1} pages`);
    return jobs;
    
  } catch (error) {
    console.warn(`Failed to scrape Save the Student:`, error instanceof Error ? error.message : String(error));
    return jobs;
  }
}

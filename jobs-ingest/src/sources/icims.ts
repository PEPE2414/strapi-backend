...
export async function scrapeICIMS(company: string, subdomain?: string): Promise<CanonicalJob[]> {
  // iCIMS career pages
  const baseUrl = subdomain ? `https://${subdomain}.icims.com` : `https://${company}.icims.com`;
  const apiUrl = `${baseUrl}/jobs/search`;
  ...
  try {
    ...
    if (!response.jobs || !Array.isArray(response.jobs)) {
      console.warn(`iCIMS API returned no jobs for ${company}`);
      return [];
    }
    ...
    console.log(`📊 iCIMS ${company}: ${jobs.length} total jobs, ${relevantJobs.length} relevant jobs`);

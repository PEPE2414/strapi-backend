  ...
  try {
    while (page <= maxPages) {
      ...
      for (const url of urls) {
        console.log(`🔄 Scraping Joblift page ${page}: ${url}...`);
        
        try {
          const { html } = await get(url);
          const $ = cheerio.load(html);
          
          // Find job listings - Joblift uses various selectors
          const jobElements = $('.job-item, .job-card, .search-result, .job-listing, [data-job-id]');
          ...
          for (let i = 0; i < jobElements.length; i++) {
            try {
              ...
              const title = jobEl.find('h2, h3, .job-title, .title, [data-job-title]').first().text().trim();
              const company = jobEl.find('.company, .employer, .job-company, [data-company]').first().text().trim();
              const location = jobEl.find('.location, .job-location, [data-location]').first().text().trim();
              const description = jobEl.find('.description, .job-description, .summary, .job-summary').first().text().trim();
              const applyLink = jobEl.find('a[href*="apply"], a[href*="job"], .apply-btn, [data-apply-url]').first().attr('href');
              const salaryText = jobEl.find('.salary, .wage, .pay, [data-salary]').first().text().trim();
              const postedDate = jobEl.find('.date, .posted, .published, [data-posted]').first().text().trim();
    ...
    console.log(`📊 Joblift: Found ${jobs.length} total jobs across ${page - 1} pages`);
    return jobs;
    
  } catch (error) {
    console.warn(`Failed to scrape Joblift:`, error instanceof Error ? error.message : String(error));
    return jobs;
  }
}

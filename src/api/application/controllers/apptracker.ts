...
export default {
  ...
  async find(ctx: any) {
    ...
    const [results, total] = await Promise.all([
      strapi.db.query('api::application.application').findMany({
        where, orderBy: sort, limit: pageSize, offset: (page - 1) * pageSize,
      }),
  ...
  async findOne(ctx: any) {
    ...
    const entity = await strapi.db.query('api::application.application').findOne({
      where: { id, owner: userId }, populate: { owner: true },
    });
  ...
  async stats(ctx: any) {
    ...
    const donut = {
      total, phase1: counts.Phase1, phase2: counts.Phase2, assessment: counts.Assessment, interview: counts.Interview,
    };
